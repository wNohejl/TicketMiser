...
public class DeskThemeTests
{
    private static readonly string Css =
        File.ReadAllText(Path.Combine(RepoRoot(), "src/TicketMiser.Desk/wwwroot/css/desk.css"));
    ...
    private static string TokenValue(string block, string token, string where)
    {
        ...
        Assert.True(match.Success, $"{token} is not declared in {where} of desk.css");
    ...
    private static string Block(string selectorPattern)
    {
        ...
        Assert.True(match.Success, $"no token block matched {selectorPattern} in desk.css");
    ...
    private static string RepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "TicketMiser.slnx")))
            dir = dir.Parent;
